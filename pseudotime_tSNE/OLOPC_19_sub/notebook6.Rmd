...
```{r}
#use top 3 branches
big3             <- order(tip_sizes_branch, decreasing=TRUE)[1:3]
big3
```
...
branches3 <- apply(
  pt@branch[, big3, drop=FALSE], 
  1, 
  function(x) {
    idx <- which(!is.na(x))[2]    # 1, 2, or 3
    if (is.na(idx)) return(NA_integer_)
    idx
  }
)
# 5) Check you really got three non‐NA groups of the expected sizes
...
# 0) Start over from edges data.frame, to be sure g is entirely fresh:
g <- graph_from_data_frame(edges, directed = FALSE)

# 1) drop any edges weight is NA or non-numeric
...
# 2) drop loops, merge parallel edges keeping the max weight
g <- simplify(
  g,
...
layout_auto <- layout_nicely(g)   # picks a reasonable default under the hood
...
# Branch 1
   OPC  COP  NFOL  MOL_DA   MOL 
  1764 1499  842   822   372  
...
Arm 1 (branches3==1) → the OPC → COP → NFOL lineage

Arm 2 (branches3==2) → one MOL trajectory

Arm 3 (branches3==3) → another MOL trajectory
